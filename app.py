...
def create_protobuf(uid):
    """Tạo protobuf message chứa UID để lấy thông tin player (Đã fix đúng field name của uid_generator_pb2)."""
    try:
        message = uid_generator_pb2.uid_generator()
        message.krishna_ = int(uid)  # Sửa từ saturn_ sang krishna_
        message.teamXdarks = 1       # Sửa từ garena sang teamXdarks
        if hasattr(message, 'ob_version'):
            message.ob_version = "OB48"
        return message.SerializeToString()
    except Exception as e:
...
def handle_requests():
    ...
    try:
        def process_request():
            ...
            encrypted_uid = enc(uid)
            if encrypted_uid is None:
                raise Exception("Mã hóa UID thất bại.")
